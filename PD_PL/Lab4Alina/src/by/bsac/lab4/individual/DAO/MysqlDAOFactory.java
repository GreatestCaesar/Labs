...
public class MysqlDAOFactory extends DAOFactory { /**  Наследование от DAOFactory, так как класс родитель может служить набором БД, к которым можно подключиться, но в нашем случае только MySQL
    ...
    public static String DB_PASSWORD = "1234"; /** Пароль от вашего MySQL ОБЯЗАТЕЛЬНО ПРОВЕРИТЬ ПЕРЕД ЗАПУСКОМ */
    public static String DB_NAME = "spacee"; /** Название БД, если она не создана, либо названа неправильно ПРОГРАММА РАБОТАТЬ НЕ БУДЕТ */
    ...
    /** Как и говорилось в описании класса, методы служат для того, чтобы показать остальным пользователям, либо тем, кто будет работать
     ...
     от требований */
    public SpaceDAO getSpaceDAO(){ return new MysqlSpaceDAO();}
}
